...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            ...
            <div class="box-content">
                ...
                <form name="Form1" id="Form1" enctype="multipart/form-data" method="post" class="form-inline" role="form">
                    ...
                    <?php
                    $cut_count= 0;
                    $SQL_cut="SELECT * FROM efl_db_pts.`tb_pc_detail` 
                              where po_no LIKE '%$po_no%' and style_no LIKE '%$style_no%' 
                              ORDER BY `tb_pc_detail`.`id`  DESC LIMIT 1";    //table name
                    ...
                    <table class="bottomBorder" border="1">
                        <tr>
                            ...
                            <th>Purchase Order: <span style="color:#F00">*</span></th>
                            <td><input name="purchase_no" id="purchase_no" readonly value="<?php echo $purchase_no;?>" required />
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td><input type="text" readonly name="cut_id" tabindex="3" placeholder="Cut No" value="<?php echo $po_no.'_'.$purchase_no.'_'.$style_no.'_'.$color.'_'.((!empty($item)) ? $item : '0').'_'.$cut_count_1.'.';?>" required autofocus /></td>
                            <th>Ordered QTY: <span style="color:#F00">*</span></th>
                                    ...
                                    <div id="size_table">
                                        ...
                                        <table class="bottomBorder" id="size_mrkr" style="box-shadow:0px 0px 0px 0px #FFF ;" border="1">
                                            <tr>
                                                ...
                                                <th>Size<input name="data1" id="data1" type="hidden" value="<?php echo "<input name='size[]' type='text' tabindex='7' onclick='select()' size='8' placeholder=''  autofocus />"; ?>" /></th>
                                                <th>Qty<input name="data5" id="data5" type="hidden" value="<?php echo "<input name='qty[]' type='text' tabindex='7' onclick='select()' size='8' placeholder=''  autofocus />"; ?>" /></th>
                                                <th>Cut Qty<input name="data6" id="data6" type="hidden" value="<?php echo "<input name='cut_qty[]' type='text' tabindex='7' onclick='select()' size='8' placeholder=''  autofocus />"; ?>" /></th>
                                                <th>Balance Qty<input name="data7" id="data7" type="hidden" value="<?php echo "<input name='balance_qty[]' type='text' tabindex='7' onclick='select()' size='8' placeholder=''  autofocus />"; ?>" /></th>
        ...
        $SQL_po="SELECT t1.*, t2.* FROM (SELECT id, po_no, purchase_order, brand, item, style_no, style_name, size, quantity,
                quality, color, ex_factory_date 
                FROM efl_db_pts.`tb_po_detail`
                WHERE po_no LIKE '%$po_no%' AND  purchase_order LIKE '%$purchase_no%' and item LIKE '%$item%' and color LIKE '%$color%') as t1
                Left JOIN
                (SELECT po_no, size as cut_size, COUNT(size) as count_cut_size_wise, 
                SUBSTRING_INDEX(SUBSTRING_INDEX(cut_tracking_no, '_', -3), '_', 1) as color, 
                SUBSTRING_INDEX(SUBSTRING_INDEX(cut_tracking_no, '_', -4), '_', 1) as style_no, 
                SUBSTRING_INDEX(SUBSTRING_INDEX(cut_tracking_no, '_', -2), '_', 1) as item
                FROM efl_db_pts.`tb_pc_detail` 
                WHERE po_no LIKE '%$po_no%'
                and SUBSTRING_INDEX(SUBSTRING_INDEX(cut_tracking_no, '_', -2), '_', 1) LIKE '%$item%' 
                and SUBSTRING_INDEX(SUBSTRING_INDEX(cut_tracking_no, '_', -3), '_', 1) LIKE '%$color%'
                GROUP BY po_no, size) as t2
                ON t1.po_no=t2.po_no and t1.style_no=t2.style_no and t1.size=t2.cut_size and t1.color=t2.color";    //table name
                                            ...
                                            while($row_po = mysql_fetch_array($result_po)) // Be careful $row1 can't repalce inside tis loop.
                                            {
                                                ...
                                                $count_cut_size_wise=$row_po['count_cut_size_wise'];
                                                $balance_qty = $quantity - $count_cut_size_wise;
                                                echo '<tr>
          ...
          <td><input name="count_cut_size_wise[]" type="text" value="'.$count_cut_size_wise.'" readonly onclick="select()" tabindex="7" size="8" placeholder="Cut Qty" required autofocus /></td>
          <td><input name="balance_quantity[]" type="text" value="'.$balance_qty.'" readonly onclick="select()" tabindex="7" size="8" placeholder="Balance Quantity" required autofocus /></td>
