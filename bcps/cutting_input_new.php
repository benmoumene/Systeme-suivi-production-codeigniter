     ...
     <!--<br /><br /><br />-->
     	...
     	<table class="bottomBorder" border="1">
            ...
            <tr>
                ...
                <td><input type="text" readonly name="cut_id" tabindex="3" placeholder="Cut No" value="<?php echo $po_no.'_'.$style_no.'_'.$color.'_'.((!empty($item)) ? $item : '0').'_'.$cut_count_1.'.';?>" required autofocus /></td>
   ...
   <div id="size_table">
    ...
    <table class="bottomBorder" id="size_mrkr" style="box-shadow:0px 0px 0px 0px #FFF ;" border="1">
        <tr>
          ...
          <th>Cut Qty<input name="data6" id="data6" type="hidden" value="<?php echo "<input name='cut_qty[]' type='text' tabindex='7' onclick='select()' size='8' placeholder='Cut Qty' required autofocus />"; ?>" /></th>
          <th>Balance Qty<input name="data7" id="data7" type="hidden" value="<?php echo "<input name='balance_qty[]' type='text' tabindex='7' onclick='select()' size='8' placeholder='Balance Cut Qty' required autofocus />"; ?>" /></th>
      ...
      <?php
         ...
         $SQL_po="SELECT t1.*, t2.* FROM (SELECT id, po_no, purchase_order, brand, item, style_no, style_name, size, quantity,
                ...
                WHERE po_no LIKE '%$po_no%' and item LIKE '%$item%' and color LIKE '%$color%') as t1
                INNER JOIN
                (SELECT po_no, size, COUNT(size) as count_cut_size_wise, 
                ...
                ON t1.po_no=t2.po_no and t1.style_no=t2.style_no and t1.size=t2.size and t1.color=t2.color";    //table name
...
//      echo '</pre>';
         ...
         while($row_po = mysql_fetch_array($result_po)) // Be careful $row1 can't repalce inside tis loop.
         {
          ...
          <td><input name="count_cut_size_wise[]" type="text" value="'.$count_cut_size_wise.'" readonly onclick="select()" tabindex="7" size="8" placeholder="Size" required autofocus /></td>
