		...
		if($pf == '')
			{
				  ...
				  foreach($_POST['part_info'] as $rowz=>$p_info)
				  { $part_info[$cnt] = $p_info ; $cnt = $cnt+1 ; }


				  $part_infom = '';
				  for($i = 1; $i < $cnt; $i++)
				  {
					if($i == $cnt-1) { $part_infom .= "'".$part_info[$i]."'" ; }
					else { $part_infom .= "'".$part_info[$i]."', " ; }
				  }
...
//	    echo '<pre>';
		 ...
		 // Execution of this Query is done after some of lines...
		 //die($SQL);
...
if($pf == 0) { $pdf->SetTitle('Solid Part: A.CutID-'.$decd_CutID); }
if($pf == 1) { $pdf->SetTitle('Print Part: A.CutID-'.$decd_CutID); }
...
// print a block of text using Write()
	...
	$srl = 1;
	$results = $obj->sql($SQL); // The Query is defined Above...
...
$top = $top+4;
	$pdf->MultiCell(25, 4.5, ' ', 0, 'L', 0, 0, 1.2, $top, true, 0, false, true, 4.5, 'L', true);
	
	$pdf->SetFont('times', 'B', 9);
	$pdf->MultiCell(35, 4.5, 'Buyer: '.$buyer[$j], 0, 'L', 0, 0, 34, $top, true, 0, false, true, 4.5, 'L', true);
	$pdf->MultiCell(32, 4.5, 'Bundle No: '.$BundleNo[$j], 0, 'L', 0, 0, 64.5, $top, true, 0, false, true, 4.5, 'L', true);
...
$top = $top+4.3;
	...
	$pdf->MultiCell(32, 4.5, 'Size: '.$Suff[$j], 0, 'L', 0, 0, 64.5, $top, true, 0, false, true, 4.5, 'L', true);
	// The above line is usually located below of GMT but it is place above to make the font BOLD.
	$pdf->SetFont('times', '', 9);
	$pdf->MultiCell(25, 4.5, 'Gmt: '.$BundleRange[$j], 0, 'L', 0, 0, 34, $top, true, 0, false, true, 4.5, 'L', true);
...
//	$pdf->MultiCell(25, 4.5, 'Cut No: '.$CutNo[$j], 0, 'L', 0, 0, 64.5, $top, true, 0, false, true, 4.5, 'L', true);
    $pdf->MultiCell(38, 4.5, 'A.CutID: '.$AutoCutID[$j], 0, 'L', 0, 0, 33, $top, true, 0, false, true, 4.5, 'L', true);
	$pdf->SetFont('times', '', 9);
	$pdf->MultiCell(67, 4.5, 'Clr: '.$Color[$j], 0, 'L', 0, 0, 1.2, $top, true, 0, false, true, 4.5, 'L', true);
	// The above line is usually located avobe of CutNo but it is place below to make the font Un-BOLD.
...
//	$pdf->MultiCell(35, 4.5, 'A.CutID: '.$AutoCutID[$j], 0, 'L', 0, 0, 1.2, $top, true, 0, false, true, 4.5, 'L', true);
	$pdf->MultiCell(25, 4.5, 'Ordr No: '.$OrderNo[$j], 0, 'L', 0, 0, 64.5, $top, true, 0, false, true, 4.5, 'L', true);
	// The above line is usually located below of LotNo but it is place above to make the font Un-BOLD.
	
	$pdf->SetFont('times', 'B', 9);
	$pdf->MultiCell(32, 4.5, 'Lot No: '.$LotNo[$j].'/ tNo-'.$RollNo[$j], 0, 'L', 0, 0, 1.2, $top, true, 0, false, true, 4.5, 'L', true);


$top = $top+1;
	$pdf->writeHTMLCell(92, 0, 0.2, $top, $line, 0, 1, 0, true, '', true); // Drawing the straight line border of the Table.

if($j+1 != $prange) {
$top = $top+3;
	$pdf->writeHTMLCell(92, 0, 5, $top, $dash, 0, 1, 0, true, '', true); // Drawing the dashed line after each box.
}
	

$top = $top+1; // It is for starting of next new line of next Table.
	$pdf->Line(1.2, $left, 1.2, $right, $style2); // Drawing the Left side border of the Table.
	$pdf->Line(88.4, $left, 88.4, $right, $style2); // Drawing the Right side border of the Table.
...
$top2 = $top2+4;
	$pdf->MultiCell(25, 4.5, ' ', 0, 'L', 0, 0, 110, $top2, true, 0, false, true, 4.5, 'L', true);
	
	$pdf->SetFont('times', 'B', 9);
	$pdf->MultiCell(35, 4.5, 'Buyer: '.$buyer[$j], 0, 'L', 0, 0, 142.8, $top2, true, 0, false, true, 4.5, 'L', true);
	
	$pdf->MultiCell(32, 4.5, 'Bundle No: '.$BundleNo[$j], 0, 'L', 0, 0, 173.3, $top2, true, 0, false, true, 4.5, 'L', true);
...
$top2 = $top2+4.3;
	...
	$pdf->SetFont('times', '', 9);
	$pdf->MultiCell(25, 4.5, 'Gmt: '.$BundleRange[$j], 0, 'L', 0, 0, 142.8, $top2, true, 0, false, true, 4.5, 'L', true);
...
//	$pdf->MultiCell(25, 4.5, 'Cut No: '.$CutNo[$j], 0, 'L', 0, 0, 173.3, $top2, true, 0, false, true, 4.5, 'L', true);
    $pdf->MultiCell(38, 4.5, 'A.CutID: '.$AutoCutID[$j], 0, 'L', 0, 0, 143, $top2, true, 0, false, true, 4.5, 'L', true);
	$pdf->SetFont('times', '', 9);
	$pdf->MultiCell(67, 4.5, 'Clr: '.$Color[$j], 0, 'L', 0, 0, 110, $top2, true, 0, false, true, 4.5, 'L', true);
	// The above line is usually located avobe of CutNo but it is place below to make the font Un-BOLD.

$top2 = $top2+4.3;
//	$pdf->MultiCell(35, 4.5, 'A.CutID: '.$AutoCutID[$j], 0, 'L', 0, 0, 110, $top2, true, 0, false, true, 4.5, 'L', true);
	$pdf->MultiCell(25, 4.5, 'Ordr No: '.$OrderNo[$j], 0, 'L', 0, 0, 173.3, $top2, true, 0, false, true, 4.5, 'L', true);
	// The above line is usually located below of LotNo but it is place above to make the font Un-BOLD.
	
	$pdf->SetFont('times', 'B', 9);
		$pdf->MultiCell(32, 4.5, 'Lot No: '.$LotNo[$j].'/ tNo - '.$RollNo[$j], 0, 'L', 0, 0, 110, $top2, true, 0, false, true, 4.5, 'L', true);
