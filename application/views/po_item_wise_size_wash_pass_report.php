<table class="table">
    <thead>
        ...
        <tr>
            ...
            <th class="center">Wash Return</th>
        </tr>
    </thead>
    <tbody>
    <?php
    ...
    foreach ($order_size as $v){ ?>
        <tr style="<?php if($v['total_cut_input_qty'] > $v['count_washing_pass']){ ?>background-color: #ff9098; <?php } if($v['total_cut_input_qty'] == $v['count_washing_pass']){ ?>background-color: #aeff82;<?php } ?>">
            <td class="center"><?php echo $v['size'];?></td>
            <td class="center"><?php echo ($v['total_cut_input_qty'] != '' ? $v['total_cut_input_qty'] : 0);?></td>
            <td class="center"><?php echo ($v['po_item_size_wise_wash_going_qty'] != '' ? $v['po_item_size_wise_wash_going_qty'] : 0);?></td>
            <td class="center" style="cursor: pointer; font-size: 18px; font-weight: 800;" data-target="#myModal" data-toggle="modal" onclick="getRemainCLs('<?php echo $v['po_no'];?>', '<?php echo $v['so_no'];?>', '<?php echo $v['purchase_order'];?>', '<?php echo $v['item'];?>', '<?php echo $v['quality'];?>', '<?php echo $v['color'];?>', '<?php echo $v['size'];?>');"><?php echo ($v['count_washing_pass'] != '' ? $v['count_washing_pass'] : 0);?></td>
        </tr>
    <?php
