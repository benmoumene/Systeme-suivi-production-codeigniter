<table class="table">
    ...
    foreach ($order_size as $v){ ?>
        <tr style="<?php if($v['total_cut_input_qty'] > $v['count_end_line_qc_pass']){ ?>background-color: #ff9098; <?php } if($v['total_cut_input_qty'] == $v['count_end_line_qc_pass']){ ?>background-color: #aeff82;<?php } ?>">
            ...
            <td class="center"><?php echo ($v['count_mid_line_qc_pass'] != '' ? $v['count_mid_line_qc_pass'] : 0);?></td>
            <td data-target="#myModal2" data-toggle="modal" class="center" style="cursor: pointer; font-size: 18px; font-weight: 800;" onclick="getRemainCLs('<?php echo $v['po_no'];?>', '<?php echo $v['so_no'];?>', '<?php echo $v['purchase_order'];?>', '<?php echo $v['item'];?>', '<?php echo $v['quality'];?>', '<?php echo $v['color'];?>', '<?php echo $v['size'];?>');"><?php echo ($v['count_end_line_qc_pass'] != '' ? $v['count_end_line_qc_pass'] : 0);?></td>
        </tr>
    <?php
